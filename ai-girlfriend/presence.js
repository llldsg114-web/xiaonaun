...
const pacingOf=(userText,reply,ctx)=>{const c=O(ctx),s=O(c.st),rs=A(reply),L=rs.join("").length,ut=String(userText||"");if(NO(s)||!L)return null;if(c.crisis||CR(ut))return{delayMs:200,typingMs:300,split:false};const ar=N(E.UE_AROUSAL[O(c.ue).type],0),en=N(O(s.moodDay).energy,.6);const ms=(M.min(2200,90*ut.length)+(ar>.3?260:620)+L*(105-30*ar)*(1.35-.55*en))*(.45+1.45*RN(c,s)());return{delayMs:M.round(M.max(320,M.min(7800,ms))),typingMs:M.round(M.min(6e3,L*80)),split:rs.length>1||(L>34&&ar>.3)};};
