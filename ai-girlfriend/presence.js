...
const G=E.mod("contingency");if(G)try{G.contingencePass(rs[0],rs,{st:s,ue:c.ue,lv:c.lv,crisis:!!c.crisis||CR(ut),text:ut,rng:RN(c,s)});}catch(e){}
...
const makeupLine=(state,ctx)=>{const s=O(state),p=P(s),q=N(p.q,0);if(!q||NO(s))return null;const t=String(MK[q-1]||"");return(!t||E.GUILT_TRIP_RE.test(t)||E.PERSONA_BREAK_RE.test(t))?null:{text:t,motive:"makeup"};};
