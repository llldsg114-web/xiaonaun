/* 小暖 v13 memory.js · T2 薄切片 | 契约 §2.3 约定 §2.1A–H | V-90 ≤14336B（T5a 12288→14336）
...
const TAGS = [[/(吃|饿|外卖|火锅|奶茶|蛋糕|甜|饭)/, "吃"], [/(累|困|睡|熬夜|加班|下班|上班|开会)/, "忙"],
 ...
 [/(程序员|职业|上班|工作|公司|老板|同事)/, "工作"]];   // 决策⑤：补 工作 类 tag
const tg = (t) => { const o = []; for (const p of TAGS) if (p[0].test(t)) o.push(p[1]); return o; };
/* 遗留-1：tag 原只认句面，「我是设计师」tags 空 → .45 加权落空 → 沉默。改喂 句面+key+value 三元。*/
...
const RULES = [[RX("(?:我叫|我的?名字[叫是])\\s*(#{1,8})"), "称呼", .85, 1],
 ...
 [RX("我是(?:个|名)?\\s*(#{1,6}(?:师|员|生|工|家|猿|媛|士))"), "工作", .8, 1],   // 遗留-1：补 猿|媛|士；下限 2→1 让 医生/护士 双字职业也抽得出
...
function extractFacts(text, state, ctx) {
 try {
  ...
  if (!ASK.test(t.trim())) for (const r of RULES) {   // 提问不是陈述：「我喜欢什么？」是考她
   ...
   if (v && v.length < 13 && !BADV.test(v)) { const k = r[1].replace("$1", m[1]); fa.push(mk(k, v, r[2], tgf(t, k, v), now)); }   // 遗留-1：三元派生 tag
  }
...
/* R25 融入式（胜负手）：事实只作从句成分，不做「我记得/你之前说」式播报；槽位只回填 value 原文，禁一切
   生成式改写 —— H11＝0% 的实现保证 */
...
/* 遗留-2 脱敏：出口闸判成句，「我喜欢机器人」无自称结构即放行 —— 沉浸感已被击穿。补入口闸净化 value：
   命中即静音（改试下一条），零改写（改写即破 H7=0）。程序员/猿/媛 折叠成「职」再判，SF 是等长折叠件。*/
const JOBX = /程序[员猿媛]/g, SF = "上班族";
const taint = (v) => E.PERSONA_BREAK_RE.test(String(v).replace(JOBX, "职"));

function weave(f, text, rng) {   // conf<.75 走试探式：句尾必带 吧/来着，把不确定摊开说而不装懂
...
/* 待决点② 横向走查表：recallV2 自 engine.js:2899 早退绕过 :3050 挂载点；engine 冻结故挂载点上移本模块
   （mod() 调用期查表，缺件返 null 落回原句，D 单向依赖不破）。nosplit＝调用方只收单条 replies[0]。
   SF 折叠：出口闸整句跑破墙表，「程序」裸子串按死程序族，等长折叠后送、回来还原。
   A6-b 配额回写：engine.js:2896 丢弃 rv.tx，宿主 textureAfterTurn 恒收 {} → 微行为不计量（实测 18/日
   vs CAP=6）。故就地按 texture.js 同口径累加 state.tex，与非 recall 共用日配额；t 留宿主自增免双计。*/
function skin(line, text, state, c, rng) {
 try {
  const T = E.mod("texture"); if (!T || typeof T.texturePass !== "function") return null;
  const j = (line.match(JOBX) || [])[0], fd = j ? line.split(j).join(SF) : line;
  const x = T.texturePass(fd, state, { rng, lv: c.lv, ue: E.detectUserEmotion(text),   // ue null 时门④按 neutral
  ...
  return { line: j ? x.text.split(SF).join(j) : x.text, tx: { kind: x.kind } };
 } catch (e) { return null; }
}
 ...
 try {
  ...
  for (const h of retrieveFacts(t, state, 3)) { const f = h.fact;
   ...
   if (line) return Object.assign({ line, mode: f.conf >= .75 ? "blend" : "probe", factId: f.id },
    skin(line, t, state, c, rng)); }   // 单轮至多 1 次；skin 返 null 即原句（Object.assign 忽略 null）
  return null;
 } catch (e) { return null; }
}
...
function migrateV12(state) {
 try {
  ...
  const pf = (k, v, at) => { const f = mk(k, String(v).slice(0, 12), .6, tgf("", k, String(v)), at); f.src = "v12"; f.lastSeenAt = at; if (!get(out.facts, f.id)) out.facts.push(f); };   // 遗留-1：迁移件同享
...
const jokeCandidates = () => [];   // R38 属 Tier2，texture 已容忍空表
