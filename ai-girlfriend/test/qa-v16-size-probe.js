...
ok(s.engine <= 248477, "engine.js ≤ 248477", `${s.engine} ≤ 248477`);
...
/* ★ v22 T-v33：三模块回让 E=40B 予 engine ⇒ engineNetMax 右移 40、moduleSumMax 左移 40。
 * 平行字面量随 SIZE_BUDGET 同步翻转（旧值见 DESIGN-v22 §5 T-v33 的替换表）。 */
ok(s.engineNet <= 2740, "engineNet ≤ 2740", `${s.engineNet} ≤ 2740`);
ok(s.moduleSum <= 28003, "moduleSum ≤ 28003", `${s.moduleSum} ≤ 28003`);
...
ok(B.engineMax === 248477 && B.engineBase === 245737 && B.engineNetMax === 2740,
  "①' 新值落位 248477 = 245737 + 2740（v22 反向路径：三模块回让 E=+40 予 engine）",
  `${B.engineMax} / ${B.engineBase} / ${B.engineNetMax}`);
...
ok(quotaSum === 28003, "②' moduleSumMax 落位 28003（v22 −40，锁② 两边同移）", String(quotaSum));
...
ok(B["memory.js"] === 13352 && B["presence.js"] === 3585 && B["texture.js"] === 4384 && B["contingency.js"] === 6682,
  "④' v22 四模块配额落位 13352 / 3585 / 4384 / 6682（三模块回让 13/13/14，contingency 逐位不动，Σ 左移至 28003）",
...
if (mV33) {
  ok(Number(mV33[1]) === B.engineMax, "⑤' V33 === engineMax（248477）",
    `V33=${mV33[1]} vs engineMax=${B.engineMax}`);
  /* v21：248537 自本轮起也进入「旧值」名单 —— 路径③ 让渡后它就是漏改针位的特征值。
   * v22：上一轮的 V33 值同样成为「旧值」，但它由 ⑤' 的 `=== B.engineMax` 严格等式直接兜住，
   *      故此处不再新增字面量 —— 旧值名单是历史遗留的显式黑名单，不作为新增机制扩张
   *      （T-v33 归零判据要求活代码内不得再出现上一轮的 V33 字面量，DESIGN-v22 §5）。 */
