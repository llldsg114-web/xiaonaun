...
/* ★★【T05 · D5 体积重标定 · 主理人 Qi 批准（DESIGN §11「T05/架构侧按新体积重设阈值」）】★★
 *     触发：T04 解除 engine.js 冻结（D5，用户「都做」已批准），engine.js 由 248436B 增至 251068B
 *       （净 +2632B，mindCtx 信封落地），使既有的 V-33 / V-90-net / 系统总量 三把锁同时越界
 *       （D5 前仅余 115B 余量，一步击穿）。
 *     本轮**只动体积锁阈值**，engine.js 与四模块源码零 diff；四锁恒等式 ①②③④ 全部保持自洽。
 *       engineNetMax  = 2740 + 2048          = 7379
 *       engineMax     = 245737 + 7379        = 253116   ← 派生，非独立真源
 *       moduleSumMax  = 28003（Σ 四模块配额不变，模块零 diff ⇒ 锁② 严格等式原值保留）
 *       totalMax      = 28003 + 253116       = 281119   ← 天花板 = moduleSumMax + engineMax（严格会计恒等式，间隙恒 0；D5 已批准，非失守）
 *     四锁复算（scanSizes 直驱，非纸面）：
 *       ① 253116 = 245737 + 7379                                      ✓
 *       ② 13352+3585+4384+6682 = 28003 = moduleSumMax                 ✓（模块零改动，Σ 逐位不变）
 *       ③ 245737 + 7379 + 28003 = 281119 = totalMax                   ✓（间隙恒 0，未分配余量一个字节都不许存在；total 闸门余 2048B 随 engine 侧余量同额传导）
 *       ④ 13352>13333(19)/3585>3566(19)/4384>4366(18)/6682>6664(18)   ✓
 *     闸门语义保持：V-33 余 2048B / V-90-net 余 2048B / total 余 2048B，仍能在 >2KB 意外膨胀时先响。
 *     ⚠ 历史块（v14~v22）逐字不动；本块仅新增并与三个真源值/行尾注释同步。 */
/* ★★【v22 · 配额重谈第二次 · 反向路径（三模块回让 engine）· 主理人批准（PRD-v22 / DESIGN-v22 §3.3）】★★
 *     与 v21（engine 让渡给 contingency）**方向相反**：v22 由三个**连续零增长**的数据模块
 *     （memory / presence / texture）回让 E=40B 给 engine，用于 P0-2 的 H13 覆盖闭合。
 *     触发需求：engine.js:1307 PERSONA_BREAK_RE 连接词枚举需追加「本质上/归根结底/说白了」，
 *     造价实测 +41B；而 v21 收线后 engine 侧余量仅 42B（engineNetMax 2700 − 实测 net 2658）
 *     ⇒ 装得下但只剩 1B，属「假安全态」，故本轮借机把三个零增长模块的冗余缓冲收归 engine。
 *
 *     ── 反向路径（三模块回让 E=40）逐级推导，四锁两边同移 ──────────────
 *       memory.js     13365 − 13 = 13352   ← 实测 13333，缓冲 32→19
 *       presence.js    3598 − 13 =  3585   ← 实测  3566，缓冲 32→19
 *       texture.js     4398 − 14 =  4384   ← 实测  4366，缓冲 32→18
 *       回让合计 E    = 13 + 13 + 14       = 40
 *       engineNetMax  = 2700 + 40          = 2740
 *       engineMax(V33)= 245737 + 2740      = 248477     ← 派生，非独立真源
 *       moduleSumMax  = 276480 − 248477    = 28003      ← 锁② 左移 40（= Σ 三项回让）
 *       contingency.js 逐位不动（6682）⇒ 锁⑧ 恒等式 4518+2164=6682 免受波及。
 *       totalMax 逐位不动（276480）⇒ 270KB 承诺继续守住，**未抬顶**。
 *
 *     ── 四锁 + ⑧ 复算（node 实算，非纸面）──────────────────────────
 *       ① 248477 = 245737 + 2740                                      ✓（加载期自证）
 *       ② 13352 + 3585 + 4384 + 6682 = 28003 = moduleSumMax           ✓（严格等式 · 明细 13352+3585=16937；+4384=21321；+6682=28003）
 *       ③ 245737 + 2740 + 28003 = 276480 ≤ 276480（松弛仍为 0）        ✓（totalMax 未抬顶）
 *       ④ᵛ²² 13352>13333(19) / 3585>3566(19) / 4384>4366(18) / 6682>6664(18)  ✓
 *       ⑧ V16_ANCHOR 4518 + NET_MAX 2164 = 6682                       ✓（派生，配额未动）
 *
 *     ★ 三模块缓冲收窄到 19/19/18 的安全性：memory/presence/texture 自 v19 起**连续三版零改动**，
 *       且 `diff=0` 硬闸（qa-v19-quota-gate.js B 段）对这三项持续生效 —— 任何字节变动都会先被
 *       硬闸拦住，不会静默吃掉缓冲。18~19B 与 contingency 落地后的 18B 同档，**口径一致**。
 *     ★ engine 侧落位：实测 net 2658 + 41 = 2699 ≤ 2740，**余 41B**（AC-1.3 要求 ≥35B，达标）。
 *
 *     ⚠ V33 多针：engineMax 一旦变动，**所有平行字面量必须同步翻转**。v22 的触达面比 v21 更大 ——
 *       `248437` 这一个字面量精确落 **7 个文件**，但本轮同时惊动了「实测值族」`2658`/`571`/`248395`
 *       （strictEqual 硬钉，v19–v21 三版 engine 零改动故从未被惊动），并集 **13 个文件**。
 *       口播可用「V33 七文件引脚」，**排期与验收一律用 13**（DESIGN-v22 §5 T-v33 / O-5）。
...
/* ★★【候选 F 预算 gating 轮 · 主理人 Qi 批准（PRD-candidate-F / DESIGN-candidate-F §9）】★★
 *   触发：候选 F（真人感精调）落地后，texture.js 实测体积由候选 E 重 baselining 的 5277B
 *         增至 **5850B**（wc -c 实测；F3 词表扩量 + F4 tic 情境化 blend + F5 门槛/ramp 微调，
 *         相对候选 E 净增 +573B；落在架构预留 6177B 天花板之内，余 327B）。
 *   路径：**同候选 E 流程 ——「配额 = 实测、缓冲 0」的钉实测值口径**（候选 E 已用满，本次沿用）。
 *   仅 texture.js 落在 SIZE_BUDGET.mods 列表内；local-heuristic.js / reply-texture-orchestrator.js /
 *   app.js 不在列表，其增长不级联（DESIGN-candidate-F §8.6 铁律），故仅 texture 一项翻数 + 级联两总项。
 *   级联（engine/memory/presence/contingency 冻结未动，模块零 diff）：
 *     texture.js    5277 → 5850（Δ +573，配额=实测，缓冲 0）
 *     moduleSumMax 28896 → 29469（= 28896 + 573，锁② 严格等式 13352+3585+5850+6682=29469）
 *     totalMax     282012 → 282585（= 282012 + 573 = moduleSumMax 29469 + engineMax 253116，间隙恒 0）
 *   相对 V14 基线净增（texture.js）：候选 E 的 920 → 1493（= 920 + 573；V14 基线 4357，5850 − 4357 = 1493）。
 *   四锁复算（scanSizes 直驱，非纸面）：
 *     ① 253116 = 245737 + 7379                                   ✓（engine 侧逐位不动）
 ...
 *   ★ 风险登记：texture.js 已钉实测 5850B（缓冲 0），未来任何字节增量必须先重谈配额；
 *     5850 < 6177 天花板，仍有 327B 余度，但按「配额=实测」纪律，5850 即为新硬锁值。
 *   ⚠ 上方 v14~候选E 历史块**逐字不动**；本块仅新增并与三个真源值同步翻转。 */
const SIZE_BUDGET = {
  ...
  "texture.js": 5850,      // 候选 F 钉实测 · 主理人批准 · 由候选 E 重 baselining 5277B 增至 5850B（实测 +573B，F3 词表扩量 + F4 tic blend + F5 门槛/ramp；配额=实测，缓冲 0；<6177 天花板，余 327B）
  ...
  moduleSumMax: 29469,     // 候选 F 级联 · 主理人批准 · 由 28896 级联 +573（texture 5277→5850 的增量）· 锁② 严格等式 13352+3585+5850+6682=29469 仍成立（模块零 diff，仅 texture 配额随实测抬升）
  totalMax: 282585,        // 候选 F 级联 · 主理人批准 · 由 282012 级联 +573（= moduleSumMax 29469 + engineMax 253116，间隙恒 0；270KB 承诺仍守住，未抬顶）
};
