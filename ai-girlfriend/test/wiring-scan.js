...
const SIZE_BUDGET = {
  ...
  engineNetMax: 2800,      // 改配额必须走代码评审 · 主理人 Qi 于 v17 批准 2400→2800（Δ=+400 · 归一化层 + R2-A5b + selfTick 防重放）· v18 不动（A2 纯模块侧路径，engine 侧不参与让渡）
  engineMax: 248537,       // 派生量 = engineBase + engineNetMax（V-33 真实硬上限；非配额项，改上面两项即自动失配，见下方自洽断言）· v18 不动 ⇒ V33 三处均不翻转
  ...
  "contingency.js": 6582,  // 改配额必须走代码评审 · 主理人 Qi 于 v18 批准 5671→6582（受援方 +911B · 残差式 27943−21361）· v20 实测 6270（T0 基线已迁移，历史 5652 仅作审计轨迹），配额余量 312B；★门禁锁④用严格 >（B[f] > T0_BYTES[f]），故自 5652 起的安全 Δ 上限是 929B 而非 930B —— 第 930B 会让「配额 > 基线」失效（业务锁用 ≤，对此毫无察觉），详见下方 v20 块
