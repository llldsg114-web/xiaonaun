...
test("A1-a 配额数字落点：memory 13352 / texture 5850 / contingency 6682 / moduleSum 29469 / net 7379 / totalMax 282585", () => {
  const B = WS.SIZE_BUDGET;
  assert.strictEqual(B["memory.js"], 13352, "v22 批准值：回让 13B 予 engine（实测 13333 + 19B 缓冲）· 源码字节零改动，diff=0 硬闸继续生效");
  assert.strictEqual(B["texture.js"], 5850, "候选 F 钉实测：texture.js 由候选 E 5277B 增至 5850B（实测 +573B，F3/F4/F5；配额=实测，缓冲 0）");
  ...
  assert.strictEqual(B.moduleSumMax, 29469, "候选 F 级联：锁② 右移 +573 = totalMax − engineMax（仅 texture 配额随实测抬升 +573；三模块回让 E=40B 予 engine 仍成立）");
  ...
  assert.strictEqual(B.totalMax, 282585, "候选 F 级联：v14 276480 + 4639 + 候选E 893 + 候选F 573 = 282585（D5 已批准，天花板 = moduleSumMax + engineMax = 29469 + 253116 = 282585，间隙恒 0）");
  ...
  /* ★★【快照翻转 · v21 配额重谈首秀 · 路径③ · 主理人 Qi 批准（DESIGN-v21 §1.1 / §1.2 表 B）】★★
   *   V33 248537 → 248437，随 engineMax 派生同步（engineNetMax 2800→2700，engine 让渡 D=−100）。
   *   与 v16/v17 两次翻转的性质完全一致：V33 **不是独立真源**，是 engineMax 的派生常量。
   *   ⚠ 但针位清单比历史注释所称的「三处」多得多 —— v21 实读工作区确认，
   *     除本行外还有 qa-v16-size-probe.js（:38/:50/:51/:77）与 **qa-v17-independent-size.js
   *     的整张 TRUTH 独立副本**（:22，且已接入 npm run test:probe）。完整清单见 DESIGN-v21 §1.2。
   *   ⚠ 两把 engine 锁继续重合（间隙恒 0），会计恒等式两端仍必须同为 0 —— 严格度逐位不放松；
   *     本轮是把那条线整体**左移** 100B（收紧），engine 侧余量 142B → 42B，代价已被主理人追认。 */
  /* ★★【快照翻转 · v22 配额重谈第二次 · 反向路径 · 主理人批准（DESIGN-v22 §3.3 / §5 T-v33）】★★
   *   V33 248437 → 248477，随 engineMax 派生同步（engineNetMax 2700→2740，三模块回让 E=+40）。
   *   与 v16/v17/v21 三次翻转的性质完全一致：V33 **不是独立真源**，是 engineMax 的派生常量。
   *   ⚠ v22 的针位并集比 v21 更大：`248437` 这一个字面量精确落 **7 文件**，但本轮 engine.js 与
   *     contingency.js **真的长了字节**（+41 / +38），连带惊动「实测值族」硬钉 2658/571/248395
   *     （v19–v21 三版 engine 零改动，故这族从未被惊动），并集 **13 文件**。
   *     口播用「V33 七文件引脚」，排期与验收一律用 13（DESIGN-v22 §5 T-v33 / O-5）。
   *   ⚠ 两把 engine 锁继续重合（间隙恒 0），会计恒等式两端仍必须同为 0 —— 严格度逐位不放松；
   *     本轮是把那条线整体**右移** 40B（放宽），engine 侧余量 42B → 41B（因同轮消费 +41B）。 */
  const V33 = 253116;
...
test("A4 体积三闸门：V-33 ≤253116B 且 V-90 net ≤7379B 且 total ≤282585B", () => {
  ...
  assert.strictEqual(net, 5331,
    "净增应为 D5 前 2699B（v22 落位）+ T04 D5 解冻 +2632B（mindCtx 信封落地，engine.js 248436→251068）= 5331B，本轮不得再涨");
