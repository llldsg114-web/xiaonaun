...
test("T2 · 体积：R-P0 相对 v13 净增 12B（≤ DESIGN 预算 16B），且三手段在当前表结构仍在", () => {
  ...
  /* ★ v16 T1 口径调整（同族原因，第二次）：手段③的人称头由 `[你我]` 扩为
   * `[你我咱它他她您]`（轴1），系动词由裸 `是` 扩为 `(?:[是算当]|作为)`（轴4）。
   * 紧邻副词字符类 `[都也还只就]` **逐位不动**（v16 未获批扩为 `[不都也还只就]`：
   * 那会改变 +190B 的已批预算落位，需另走配额评审，详见 §T1 遗留）。
   * 旧的逐字面量断言会把「扩展」误报成「回退」，故改钉**结构骨架 + 必含元素**：
   * 既锁住 v14 R-P0 的人称绑定形态没被拆掉，也锁住 v16 四轴不会哪天被缩回去
   * —— 严格度只增不减（由 2 条断言扩为 4 条）。 */
  assert.ok(/\[你我咱它他她您\]们\?\(\?:不过\?\|其实\|/.test(src), "R-P0 手段③人称泛化被回退");
  for (const p of ["咱", "它", "他", "她", "您"]) {
    assert.ok(new RegExp("\\[你我[^\\]]*" + p).test(src), `v16 轴1 人称「${p}」被回退`);
  }
  assert.ok(/\[都也还只就\]\{0,2\}\(\?:\[是算当\]\|作为\)\.\{0,8\}/.test(src),
    "Q-V15-1 副词槽 / v16 轴4 系词被回退：应为 `[都也还只就]{0,2}(?:[是算当]|作为).{0,8}`");
