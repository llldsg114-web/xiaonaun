...
test("AC-C5-6 · contingency.js R-C5 增量 ≤470B、R-S2 净增/总量双锁均从 SIZE_BUDGET 派生；体积四锁自洽", () => {
  ...
  assert.strictEqual(B["texture.js"], 5850, "候选 F 钉实测：texture.js 由候选 E 重 baselining 5277B 增至 5850B（实测 +573B，F3 词表扩量 + F4 tic blend + F5 门槛/ramp；配额=实测，缓冲 0；<6177 天花板）");
  ...
  assert.strictEqual(B.moduleSumMax, 29469, "候选 F 级联：moduleSumMax 29469 = totalMax(282585) − engineMax(253116)（仅 texture 配额随实测抬升 +573，Σ 逐位不变）");
   ...
   * texture.js 因候选 E 合法扩展（5277B，+893B）+ 候选 F 精调（5850B，+573B，F3/F4/F5），
   * 相对 V14 基线净增量由 920 → 1493（= 候选E 5277B + 候选F 573B = 5850B，相对 V14 4357B 净增 1493）。
   * 严格等式保留：未来若 texture.js 意外膨胀，这条断言仍先响。 */
  const EXPECT = { "memory.js": -38, "presence.js": 9, "texture.js": 1493 };
